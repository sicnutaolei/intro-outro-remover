...
界面的后台线程管道另有一组同类的 `#[ignore]` 用例（用同一套 `IOR_*` 环境变量），
覆盖：工具检查 → 探测 → 检测 → 切割的全程消息转发、抽帧预览落盘、
预览与批量任务并发时消息不丢、取消标志能变成一条错误消息。

```bash
cargo test --workspace -- --ignored --nocapture
```

`crates/gui` 是**二进制** crate，`tests/` 下的集成测试没法 import 它的模块
（二进制不产出可链接的 rlib），所以这组用例写在 `worker.rs` 的内联 `#[cfg(test)]`
模块里 —— 这也是本项目没给 `gui` 加 `tests/` 目录的原因。

> **素材的边界**：`make_test_clips.sh` 造的画面是**整集一种纯色**，同一集任意两个
> 时间点抽出来的帧逐字节相同。所以这组用例能验证「抽帧成功、文件唯一、消息不丢」，
> **验证不了「抽的是不是指定时间点的画面」**。想验后者得先把素材换成带画面运动的
> （例如把 `color=` 换成 `testsrc`）。
